const parseXML = require('util').promisify(new (require('xml2js')).Parser().parseString);
...
class TCMB_DOVIZ {
    async DovizListesi() {
        let sitex = await require("node-fetch")('http://www.tcmb.gov.tr/kurlar/today.xml').then(response => response.text())
        if (!sitex) throw new Error('tcmb-doviz> TCMB\'ye erişilemedi.');
        let data = await parseXML(sitex);
