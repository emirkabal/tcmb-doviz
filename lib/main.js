...
class TCMB_DOVIZ {
    async DovizListesi() {
        const tcmb = await fetch('http://www.tcmb.gov.tr/kurlar/today.xml').then(response => response.text())
        if (!tcmb) throw new Error('tcmb-doviz> TCMB\'ye erişilemedi.');
        let data = await parseXML(tcmb);
